...
public interface Mix<M> extends ReceivePort<M> {
    ...
    <T extends ReceivePort<? extends M>> void add(final T... items) throws InterruptedException;

    /**
     * @param items If {@code null} or empty, all items will be removed.
     */
    <T extends ReceivePort<? extends M>> void remove(final T... items) throws InterruptedException;

    /**
     * @param items If {@code null} or empty, all items will be removed.
     */
    <T extends ReceivePort<? extends M>> Map<T, State> getState(final T... items);

    /**
     * @param items If {@code null} or empty, all items state will be set to {@code state}.
     */
    <T extends ReceivePort<? extends M>> void setState(final State state, final T... items) throws InterruptedException;

    <T extends ReceivePort<? extends M>> void setState(final Map<T, State> states) throws InterruptedException;

    SoloEffect getSoloEffect();

    void setSoloEffect(final SoloEffect effect) throws InterruptedException;
 }
